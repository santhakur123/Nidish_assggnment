/* ============================================================
   ██████╗ ENV CONFIG — Change this if your backend URL changes
   ============================================================
   When deploying to production, replace the URL below with
   your live backend URL, e.g.  https://your-api.onrender.com
   ============================================================ */
...
function updatePreview({ heading, paragraph, backgroundImage, textColor }) {
  ...
  void previewCardInner.offsetWidth; // reflow
  ...
  // Update paragraph (rich text HTML)
...
form.addEventListener('submit', async (e) => {
  ...
  // Sync Quill content into hidden input
  const paragraphHtml = quill.root.innerHTML;
  ...
  try {
    ...
    formSuccess.textContent    = '❌ Unable to reach the server. Make sure the backend is running on ' + API_BASE_URL;
    formSuccess.style.background  = 'rgba(244, 63, 94, 0.12)';
    formSuccess.style.borderColor = 'rgba(244, 63, 94, 0.3)';
    formSuccess.style.color       = '#f43f5e';
    formSuccess.classList.add('visible');
